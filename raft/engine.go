...
func (w *ssWorker) handle(j job) error {
	if j.task.Recover {
		...
		plog.Infof("ssWorker do task save")
		return w.save(j)
...
func (e *engine) processSteps(workerID uint64,
	...
	if atomic.CompareAndSwapInt32(&e.firstFlush, 0, 1) {
		plog.Infof("prepare to replay raft entry")
		...
		for cid := range active {
			...
			uds, hasUpdate, err := node.getUnflushed()
			plog.Infof("replay raft entry. hasUpdate:%v err:%v", hasUpdate, err)
			if err != nil {
				return err
			}
			if hasUpdate {
				nodeUpdates = append(nodeUpdates, uds...)
			}
		}
		if len(nodeUpdates) > 0 {
			if err := e.applySnapshotAndUpdate(nodeUpdates, nodes, true, true); err != nil {
				return err
			}
		}
	}

	//plog.Infof("active : %v", active)
	nodeUpdates = nodeUpdates[:0]
	for cid := range active {
		node, ok := nodes[cid]
		if !ok || node.stopped() {
	...
	if err := e.applySnapshotAndUpdate(nodeUpdates, nodes, true, false); err != nil {
		return err
	}
	...
	if err := e.applySnapshotAndUpdate(nodeUpdates, nodes, false, false); err != nil {
		return err
	}
	for _, ud := range nodeUpdates {
		node := nodes[ud.ClusterID]
...
func (e *engine) applySnapshotAndUpdate(updates []pb.Update,
	nodes map[uint64]*node, fastApply bool, taskReplay bool) error {
	notifyCommit := false
	for _, ud := range updates {
		...
		//plog.Infof("node info node.pushedIndex:%d, node.appliedIndex:%d, node.confirmedIndex:%d", node.pushedIndex, node.appliedIndex, node.confirmedIndex)
		node.applyRaftUpdates(ud, taskReplay)
	}
	if !notifyCommit {

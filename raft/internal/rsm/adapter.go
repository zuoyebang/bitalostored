...
type IStateMachine interface {
	Open(<-chan struct{}) (uint64, uint64, error)
...
func (i *InMemStateMachine) Open(stopc <-chan struct{}) (uint64, uint64, error) {
...
// Open opens the state machine.
func (s *ConcurrentStateMachine) Open(stopc <-chan struct{}) (uint64, uint64, error) {
...
// Open opens the state machine.
func (s *OnDiskStateMachine) Open(stopc <-chan struct{}) (uint64, uint64, error) {
	if s.opened {
		panic("Open invoked again")
	}
	s.opened = true
	applied, flushIndex, err := s.sm.Open(stopc)
	return applied, flushIndex, errors.WithStack(err)
}

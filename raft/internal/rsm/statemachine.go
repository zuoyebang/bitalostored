...
type StateMachine struct {
	...
	onDiskIndex          uint64
	syncedIndex          uint64
	flushInitIndex       uint64
	mu                   sync.RWMutex
	sct                  config.CompressionType
	onDiskSM             bool
	aborted              bool
	isWitness            bool
	configChangeRejected bool
}
...
func (s *StateMachine) RegisterInitsMembers(n pb.Membership) {
	s.members.set(n)
}

func (s *StateMachine) GetOnDiskIndex() uint64 {
	if s.onDiskSM {
		return s.onDiskIndex
	}
	return 0
}
...
func (s *StateMachine) apply(ss pb.Snapshot, init bool) {
	index := ss.Index
	plog.Infof("%s recovering from %s, init %t, snapshot Imported: %s", s.id(), s.ssid(index), init, ss.Imported)
...
func (s *StateMachine) OpenOnDiskStateMachine() (uint64, error) {
	...
	defer s.mu.Unlock()
	s.tryInjectTestFS()
	index, flushIndex, err := s.sm.Open()
	if err != nil {
	...
	plog.Infof("%s opened disk SM, index %d, flushIndex:%d", s.id(), index, flushIndex)
	s.onDiskInitIndex = index
	s.onDiskIndex = index
	s.flushInitIndex = flushIndex
	return index, nil
}
...
func (s *StateMachine) SetLastApplied(index uint64) {
	s.lastApplied.Lock()
	defer s.lastApplied.Unlock()
	plog.Infof("StateMachine SetLastApplied : %d", index)
...
func (s *StateMachine) setLastApplied(entries []pb.Entry) {
	if len(entries) > 0 {
		...
		if s.lastApplied.index+1 != entries[0].Index {
			plog.Panicf("%s, gap between batches, %d, %d", s.id(), s.lastApplied.index+1, entries[0].Index)
		}
...
func (s *StateMachine) save(r SSRequest) (pb.Snapshot, SSEnv, error) {
	...
	startTime := time.Now()
	meta, err := s.prepare(r)
	plog.Infof("s.prepare cost: %s", FmtDuration(time.Now().Sub(startTime)))
...
func (s *StateMachine) prepare(r SSRequest) (SSMeta, error) {
	...
	if s.Concurrent() && !s.savingDummySnapshot(r) {
		...
		plog.Infof("StateMachine do s.sm.Prepare()")
		if err != nil {
			return SSMeta{}, err
		}
	}
...
func (s *StateMachine) doSave(meta SSMeta) (pb.Snapshot, SSEnv, error) {
	...
	plog.Infof("s.snapshotter.Save cost: %s", FmtDuration(time.Now().Sub(startTime)))
	...
	for idx := range t {
		...
		if batch && update && noop {
			...
			for i := range entries {
				...
				if entries[i].IsConfigChange() && !s.configChangeRejected {
		...
		if membershipSnapshot {
			s.node.DoSnapshotForMemberships()
		}
	}
	return nil
}
...
func (s *StateMachine) entryNeedFlushed(index uint64) bool {
	if s.flushInitIndex >= s.onDiskInitIndex {
		return false
	}
	return index > s.flushInitIndex && index <= s.onDiskInitIndex
}
...
func (s *StateMachine) GetFlushInitIndex() uint64 {
	if s.onDiskSM {
		return s.flushInitIndex
	}
	return 0
}
...
func (s *StateMachine) handleEntry(e pb.Entry, last bool) error {
	defer func() {
		s.mu.Lock()
		defer s.mu.Unlock()
		plog.Infof("handleEntry type: %v, t:%d, i:%d, k:%d", e.Type.String(), e.Term, e.Index, e.Key)
		s.sm.Update(sm.Entry{Index: e.Index, Cmd: nil})
	}()
	...
	if !e.IsSessionManaged() {
		...
		if e.IsNewSessionRequest() {
			...
			if s.entryNeedFlushed(e.Index) {
				_, _, _, err := s.update(e)
				if err != nil {
					return err
				}
			} else if !s.entryInInitDiskSM(e.Index) {
...
func (s *StateMachine) handleBatch(input []pb.Entry, ents []sm.Entry, applied uint64) error {
	if len(ents) != 0 {
	...
	skipped := 0
	for _, e := range input {
		if s.entryNeedFlushed(e.Index) || (!s.entryInInitDiskSM(e.Index) && e.Index > applied) {

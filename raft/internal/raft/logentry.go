...
type entryLog struct {
	...
	processed uint64
	// ondisk init apply index
	ondisInitIndex uint64
}

func newEntryLog(logdb ILogDB, rl *server.InMemRateLimiter) *entryLog {
	firstIndex, lastIndex := logdb.GetRange()
	plog.Infof("new log entry firstindex: %d, lastIndex: %d", firstIndex, lastIndex)
	l := &entryLog{
		logdb:     logdb,
...
func (l *entryLog) setLogEntryOnDiskIndex(index uint64) {
	l.ondisInitIndex = index
	l.processed = index
	//l.ondisInitIndex = 0
}

func (l *entryLog) getLogEntryCommitedIndex() uint64 {
	return l.committed
}
...
func (l *entryLog) getEntriesSkipCheck(low uint64,
	high uint64, maxSize uint64) ([]pb.Entry, error) {
	if low == high {
		return nil, nil
	}
	plog.Infof("getEntriesFromLogDB(replay) low:%d high:%d maxSize:%d(B)", low, high, maxSize)
	ents, _, err := l.getEntriesFromLogDB(low, high, maxSize)
	if err != nil {
		return nil, err
	}
	return ents, nil
...
func (l *entryLog) firstNotAppliedIndex() uint64 {
	return max(l.processed+1, max(l.firstIndex(), l.ondisInitIndex+1))
}
...
func (l *entryLog) getEntriesForFlush(toApplyIndex, updateIndex uint64) ([]pb.Entry, error) {
	limit := maxEntriesToApplySize
	if toApplyIndex < updateIndex {
		ents, err := l.getEntriesSkipCheck(toApplyIndex,
			updateIndex, limit)
		if err != nil {
			return nil, err
		}
...
func (l *entryLog) matchEntryTerm(index uint64, term uint64) (bool, error) {
	lt, err := l.term(index)
	if err != nil {
		return false, err
	}
	if lt > 0 {
		return term >= lt, nil // fix: membership timeout
	} else {
		return term == lt, nil
	}
}

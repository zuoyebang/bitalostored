...
func newRaft(c config.Config, logdb ILogDB, initAddress ...PeerAddress) *raft {
	...
	if len(members.Addresses) <= 0 {
		plog.Infof("nodestate from initAddress member raft state:%v, members:%v", st, initAddress)
		for i, _ := range initAddress {
			r.remotes[initAddress[i].NodeID] = &remote{next: 1}
		}
		r.hasSnapshot = false
	} else {
		if _, ok := members.Removed[c.NodeID]; !ok {
...
func (r *raft) isSingleNormalActive() bool {
	if !r.isLeader() {
		return false
	}
...
func (r *raft) nodesPropose() []uint64 {
	nodes := make([]uint64, 0, r.numVotingMembers()+len(r.nonVotings))
	for id := range r.remotes {
		nodes = append(nodes, id)
	}
...
func (r *raft) setSelfRecover(recover bool) {
	plog.Infof("set self recover:%t", recover)
	r.recover = recover
}
...
//func (r *raft) getLogCommitedIndex() uint64 {
//	return r.log.committed
//}
//
//func (r *raft) setLogCommitedIndex(diskIndex uint64) {
//	if diskIndex > r.log.committed {
//		panic("diskIndex not bigger than r.log.committed")
//	}
//	r.log.committed = diskIndex
//}
...
func (r *raft) sendReplicateMessage(to uint64) {
	...
	if rp.isPaused() || !rp.isActive() {
		return
	}

	//if r.transportCheckRl != nil {
	//	isTcpFull, err := r.transportCheckRl(r.clusterID, to)
	//	if err != nil {
	//		//plog.Errorf("%s failed to get the remote tcp ratelimter, clusterId: %d, toNodeId: %d, err :%s", r.describe(), r.clusterID, to, err.Error())
	//		return
	//	}
	//	if isTcpFull {
	//		plog.Warningf("%s sendReplicateMessage tcp ratelimter, clusterId: %d, toNodeId: %d", r.describe(), r.clusterID, to)
	//		return
	//	}
	//	plog.Infof("%s sendReplicateMessage tcp ratelimter, clusterId: %d, toNodeId: %d, isFull : %v", r.describe(), r.clusterID, to, isTcpFull)
	//}
...
func (r *raft) broadcastReplicateMessagePropose() {
	r.mustBeLeader()
	for nid := range r.nonVotings {
		if nid == r.nodeID {
			plog.Panicf("%s nonVoting is broadcasting Replicate msg", r.describe())
	...
	for _, nid := range r.nodesPropose() {
		if nid != r.nodeID {
			r.sendReplicateMessage(nid)
		}
	}
...
func (r *raft) sendHeartbeatMessage(to uint64,
	...
	commmitEntry := make([]pb.Entry, 1)
	commmitEntry[0].Index = r.log.committed
	r.send(&pb.Message{
		To:       to,
		Type:     pb.Heartbeat,
		Commit:   commit,
...
// 原来的，tryCommit
func (r *raft) tryCommitOrigine() (bool, error) {
	r.mustBeLeader()
...
func (r *raft) tryCommit() (bool, error) {
	...
	for _, v := range r.witnesses {
		...
		r.matched[index] = maxMatch
		index++
	}
	r.sortMatchValues()

	var q uint64 = 0
	idx := len(r.remotes) + nAWtiness - r.quorum()
	if idx >= len(r.matched) {
		q = r.matched[len(r.matched)-1]
	} else if idx >= 0 {
		q = r.matched[idx]
	}
	// see p8 raft paper
...
func (r *raft) appendEntries(entries []pb.Entry) error {
	...
	if r.isSingleNodeQuorum() || r.isSingleNormalActive() {
...
func (r *raft) handleHeartbeatMessage(m *pb.Message) error {
	...
	comm := r.log.committed
	if len(m.Entries) > 0 {
		comm = m.Entries[0].Index
	}

	order.G_NodeSates.SetStates(r.clusterID, comm, r.log.lastIndex(), m.Term, time.Now().UnixNano(), r.isLeader(), r.isWitness())
	return nil
}
...
func (r *raft) handleInstallSnapshotMessage(m *pb.Message) error {
	...
	r.setSelfRecover(true)
	ok, err := r.restore(m.Snapshot)
	if err != nil {
		r.setSelfRecover(false)
		return err
	}
	if ok {
...
func (r *raft) handleReplicateMessage(m *pb.Message) error {
	...
	ok, err := r.log.matchEntryTerm(m.LogIndex, m.LogTerm)
	if err != nil {
		return err
	}
	if ok {
...
func (r *raft) onMessageTermNotMatched(m *pb.Message) bool {
	...
	if r.dropRequestVoteFromHighTermNode(m) {
		plog.Warningf("%s dropped RequestVote at term %d from %s, msg type :%s, leader available",
			r.describe(), m.Term, NodeID(m.From), m.Type.String())
		return true
	}
	if m.Term > r.term {
		if !isPreVoteMessageWithExpectedHigherTerm(m) {
			...
			if r.isNonVoting() {
				...
				if m.Type == pb.RequestVote {
					plog.Warningf("%s become followerKE after receiving higher term from %s leaderID:%d",
						r.describe(), NodeID(m.From), leaderID)
					...
					plog.Warningf("%s become follower after receiving higher term from %s leaderID:%d",
						r.describe(), NodeID(m.From), leaderID)
...
func (r *raft) hasConfigChangeToApply() bool {
	...
	plog.Infof("r.log.committed :%d , r.getApplied() : %d", r.log.committed, r.getApplied())
...
func (r *raft) handleNodeElection(m *pb.Message) error {
	//plog.Infof("handleNodeElection msg term: %v", m)
	if !r.isLeader() {
...
func (r *raft) handleNodeRequestVote(m *pb.Message) error {
	...
	var err error
	if r.isWitness() {
		isUpToDate = r.upToDateWitness(m.LogIndex, m.LogTerm)
	} else {
		isUpToDate, err = r.log.upToDate(m.LogIndex, m.LogTerm)
		if err != nil {
			return err
...
func (r *raft) handleLeaderHeartbeat(m *pb.Message) error {
	...
	order.G_NodeSates.SetStates(r.clusterID, r.log.committed, r.log.lastIndex(), m.Term, time.Now().UnixNano(), r.isLeader(), r.isWitness())
	return nil
}
...
func (r *raft) handleLeaderPropose(m *pb.Message) error {
	...
	//r.broadcastReplicateMessage()
	r.broadcastReplicateMessagePropose()
	return nil
}
...
func (r *raft) upToDateWitness(index uint64, term uint64) bool {
	nHisTerm := order.G_NodeSates.GetTerm(r.clusterID)
	nIdx := order.G_NodeSates.GetCommit(r.clusterID)

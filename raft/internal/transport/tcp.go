...
func (t *TCP) GetConnection(ctx context.Context,
	...
	//rate := 128 << 20
	//rb := ratelimit.NewBucketWithRate(float64(rate), int64(rate)*2)
	//wb := ratelimit.NewBucketWithRate(float64(rate), int64(rate)*2)
	//return NewTCPConnection(conn, rb, wb, t.encrypted), nil

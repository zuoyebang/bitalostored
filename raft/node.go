...
import (
	...
	"github.com/zuoyebang/bitalostored/raft/internal/transport"
...
func newNode(peers map[uint64]string,
	...
	rn := &node{
		...
		flushCallback: config.FlushCallback,
	}
...
func (n *node) resetOnDiskIndexToLogEntry() {
	onDiskIndex := n.sm.GetOnDiskIndex()
	plog.Infof("ondiskindex index :%d, include log.onDiskInitIndex", onDiskIndex)
	n.p.SetInitLogEntryOnDiskIndex(onDiskIndex)
}

func (n *node) restOnDiskIndexToSmIndexAndLastApplied() {
	onDiskIndex := n.sm.GetOnDiskIndex()
	plog.Infof("origin ondisk index :%d, include statemachine.lastapplied.index", onDiskIndex)
	n.sm.SetLastApplied(onDiskIndex)
	// n.pushedIndex = onDiskIndex
}
...
func (n *node) pushEntries(ents []pb.Entry, taskReplay bool) {
	if len(ents) == 0 {
		return
	}
	n.pushTask(rsm.Task{Entries: ents, Replay: taskReplay}, false)
...
func (n *node) replayLog(clusterID uint64, nodeID uint64) (bool, error) {
	...
	plog.Infof("%s replaying raft logs, snapshot info : %+v", n.id(), ss)
	...
	plog.Infof("ReadRaftState firstIndex:%d entryCount:%d commit:%d", rs.FirstIndex, rs.EntryCount, rs.State.Commit)
	n.logReader.SetRange(rs.FirstIndex, rs.EntryCount)
	f, l := n.logReader.GetRange()
	plog.Infof("replay log firstIndex:%d lastIndex:%d", f, l)
...
func (n *node) doSave(req rsm.SSRequest) (uint64, error) {
	...
	plog.Infof("n.sm.Save cost: %s", FmtDuration(time.Now().Sub(startTime)))
	...
	plog.Infof("n.snapshotter.Commit cost: %s", FmtDuration(time.Now().Sub(startTime)))
	if req.Exported() {
		return ss.Index, nil
	}
	...
	plog.Infof("n.logReader.CreateSnapshot cost: %s", FmtDuration(time.Now().Sub(startTime)))
...
func (n *node) recover(rec rsm.Task) (_ uint64, err error) {
	order.G_NodeSates.SetRecover(n.clusterID, 1)
	defer order.G_NodeSates.SetRecover(n.clusterID, 2)

	n.snapshotLock.Lock()
	defer n.snapshotLock.Unlock()
	if rec.Initial && n.OnDiskStateMachine() {
		plog.Infof("%s on disk SM is beng initialized", n.id())
		idx, err := n.sm.OpenOnDiskStateMachine()
		n.resetOnDiskIndexToLogEntry()
		if err != nil {
			if openAborted(err) {
	...
	n.p.SetSelfRecover(true)
	defer n.p.SetSelfRecover(false)
	ss, err := n.sm.Recover(rec)
	plog.Infof("recover snapshot is emtpy : %v", pb.IsEmptySnapshot(ss))
	if err != nil {
		if recoverAborted(err) {
			plog.Warningf("%s aborted recovery， err : %s", n.id(), err.Error())
			return 0, nil
		}
...
func (n *node) removeLog() error {
	if n.ss.hasCompactLogTo() {
		...
		if n.p.ForbidRemoveLog(compactTo) {
			plog.Infof("remove log is forbidden. remote log is not enough")
			return nil
		}
		...
		if n.flushCallback != nil {
			n.flushCallback(compactTo)
		}
		plog.Infof("remove log. node:%s compactTo:%d", n.id(), compactTo)
...
func (n *node) getUpdate() (pb.Update, bool, error) {
	...
	//plog.Infof("getUpdate has moreEntries :%v, n.confirmedIndex != n.appliedIndex : %v, n.ss.hasCompactLogTo():%v, n.ss.hasCompactedTo():%v", moreEntries, n.confirmedIndex != n.appliedIndex, n.ss.hasCompactLogTo(), n.ss.hasCompactedTo())
	if n.p.HasUpdate(moreEntries) ||
		...
		//plog.Infof("log.commitedIndex : %d, n.confirmedIndex: %d, n.appliedIndex: %d, ondiskIndex：%d", n.p.GetInitLogEntryCommitedIndex(), n.confirmedIndex, n.appliedIndex, n.sm.GetOnDiskIndex())
		ud, err := n.p.GetUpdate(moreEntries, n.appliedIndex)
		//plog.Infof("GetUpdate ud : %v", ud.UpdateCommit)
		if err != nil {
...
func (n *node) getFlushUpdate() ([]pb.Update, bool, error) {
	if n.sm.GetFlushInitIndex() == 0 {
		return []pb.Update{}, false, nil
	}
	uds, err := n.p.GetUpdateForFlush(n.sm.GetFlushInitIndex()+1, n.sm.GetOnDiskIndex()+1)
...
func (n *node) applyRaftUpdates(ud pb.Update, taskReplay bool) {
	if taskReplay {
		n.pushEntries(ud.CommittedEntries, taskReplay)
	} else {
		n.pushEntries(pb.EntriesToApply(ud.CommittedEntries, n.pushedIndex, true), taskReplay)
	}
}
...
func (n *node) updateAppliedIndex() uint64 {
	...
	//plog.Infof("n.appliedIndex :%d", n.appliedIndex)
...
func (n *node) stepNode() (pb.Update, bool, error) {
	n.raftMu.Lock()
	defer n.raftMu.Unlock()
	//plog.Infof("stepNode n.initialized()：%v", n.initialized())
...
func (n *node) getUnflushed() ([]pb.Update, bool, error) {
	n.raftMu.Lock()
	defer n.raftMu.Unlock()

	if n.initialized() {
		uds, hasUpdate, err := n.getFlushUpdate()
		if err != nil {
			return []pb.Update{}, false, err
		}
		return uds, hasUpdate, nil
	}
	return []pb.Update{}, false, nil
}
...
// memberships持久化到disk，每次配置变更，都打一份快照
func (n *node) DoSnapshotForMemberships() {
	if _, valid := n.getLeaderID(); !valid {
		plog.Warningf("no leader, cluster is not ready, doSnapshotForMemberships failed")
		return
	}
	startTime := time.Now()
	//ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	//defer cancel()
	//保证此处不清理log，快速保存snapshot
	opt := SnapshotOption{
		OverrideCompactionOverhead: false,
	}
	//timeout, _ := getTimeoutFromContext(ctx)
	_, err := n.requestSnapshot(opt, 1)
	plog.Infof("handleConfigChange doSnapshotForMemberships begin, timeout ticker: %v", 1)
	if err != nil {
		plog.Panicf("handleConfigChange doSnapshotForMemberships to update memberships on disk, err:%s", err.Error())
	}
	//_, err = getRequestState(ctx, rs)
	//if err != nil {
	//	plog.Panicf("handleConfigChange doSnapshotForMemberships wait getRequestState to update memberships on disk err:%s", err.Error())
	//}
	plog.Infof("handleConfigChange doSnapshotForMemberships cost: %s", FmtDuration(time.Now().Sub(startTime)))
}
...
func (n *node) setInitialStatus(index uint64) {
	...
	n.ss.setIndex(index)
	n.restOnDiskIndexToSmIndexAndLastApplied()

	//空snapshot，membership从
	if !n.p.HasSnapshot() {
		ms := pb.Membership{
			ConfigChangeId: 1,
			Addresses:      n.initpeers,
		}
		plog.Infof("node recover no snapshot, register init membership: %v", ms)
		n.sm.RegisterInitsMembers(ms)
		if len(ms.Addresses) > 0 {
			n.notifyConfigChange()
		}
	}
	n.setInitialized()
...
func (n *node) processRecoverStatus() bool {
	if n.ss.recovering() {
		...
		plog.Infof("processRecoverStatus Task rec.Initial: %v, rec.Save:%v, rec.Index:%v", rec.Initial, rec.Save, rec.Index)
		if !ok {
			return true
		}
		if rec.Save {

...
export const login$ = ({username, password}) => ajaxPost(process.env.BASE_URL + 'login', {username, password})
export const getHomepageData = () => ajax('/homepage')
export const handleException = (params) => ajax.post('/handleexception', {...params})

// home
export const getHomeData$ = () => ajaxGetJSON<CommonResponse<HomeDataResponse>>(process.env.BASE_URL + 'topom?forward=' + getBitalosproxyName())
export const getHomeStats$ = () => ajaxGetJSON(process.env.BASE_URL + `api/topom/stats/${getBitalosproxyXName()}?forward=` + getBitalosproxyName())
...
// v8 slot action
export const V8SlotHistoryApi = () => ajaxGetJSON<CommonResponse<HomeDataResponse>>(process.env.BASE_URL + `api/topom/slots/action/history?forward=${getBitalosproxyName()}`)
...
export const groupDeraft = (gid: number, addr: string, token: number | string) => ajaxPut(`/api/topom/group/deraft/${getXAuth(getBitalosproxyName())}/${gid}/${addr}/${token}?forward=${getBitalosproxyName()}`)
...
export const getLog = (address, ip, query, queryTime) => ajax.post(`/logquery?forward=${(ip)}`, {address, query, queryTime})
...
export const searchSlot = (key):AxiosPromise<any> => ajax(`/api/topom/tools/whichgroupkey/${key}?forward=${getBitalosproxyName()}`)
export const transferSlot = ({from,to,group})=>ajaxPut(`/api/topom/slots/action/transfer/${getXAuth(getBitalosproxyName())}/${from}/${to}/${group}?forward=${getBitalosproxyName()}`)
export const removedata = ({from,to,group,token})=>ajaxPut(`/api/topom/slots/action/removedata/${getXAuth(getBitalosproxyName())}/${from}/${to}/${group}/${token}?forward=${getBitalosproxyName()}`)
...
export const autoCompact = (server: string, value: number) => ajaxPut(`/api/topom/group/auto-compact/${getBitalosproxyXName()}/${server}/${value}?forward=${getBitalosproxyName()}`)
...
export const enableAll = (token) => ajaxPut(`/api/topom/group/replica-groups-all/${getBitalosproxyXName()}/1/${token}?forward=${getBitalosproxyName()}`)
export const disableAll = (token) => ajaxPut(`/api/topom/group/replica-groups-all/${getBitalosproxyXName()}/0/${token}?forward=${getBitalosproxyName()}`)
export const enableAcAll = () => ajaxPut(`/api/topom/group/auto-compact-all/${getBitalosproxyXName()}/1?forward=${getBitalosproxyName()}`)
export const disableAcAll = () => ajaxPut(`/api/topom/group/auto-compact-all/${getBitalosproxyXName()}/0?forward=${getBitalosproxyName()}`)
...
export const deraft = (cloud, token) => ajaxPut(`/api/topom/group/deraftcluster/${getXAuth(getBitalosproxyName())}/${cloud}/${token}?forward=${getBitalosproxyName()}`)
...
//dk
export const getDkList = () => ajax.get<CommonResponse<any>>(`/api/topom/dk/list/${getBitalosproxyXName()}?forward=${getBitalosproxyName()}`)
export const createDk = (dk: DkItem) => ajax.put(`/api/topom/dk/create/${getBitalosproxyXName()}?forward=${getBitalosproxyName()}`, dk)
export const removeDK = (key) => ajax.put(`/api/topom/dk/remove/${key}/${getBitalosproxyXName()}?forward=${getBitalosproxyName()}`)

// migrate table
export const getMigrateTable$ = () => ajaxGetJSON(process.env.BASE_URL + `api/topom/migratelist/${getBitalosproxyXName()}?forward=${getBitalosproxyName()}`)

...
`
	...
	if _, err := redis.String(c.Do("SCRIPT", "LOAD", "[")); err == nil {
		t.Fatal("error return")
	} else if err.Error() != "ERR Error compiling script (new function):L <script> line:1(column:1) near '[':   syntax error" {
		t.Fatal(err)
	}
	...
	// Test Evalsha
	script1sha2 := "bfbf458525d6a0b19200bfd6db3af481156b367b"
	if ok, err := redis.String(c.Do("SCRIPT", "LOAD", "return {KEYS[1],ARGV[1]}")); err != nil {
		t.Fatal(err)
	} else if ok != script1sha2 {
		t.Fatal(ok)
	}

	if ok, err := redis.Strings(c.Do("evalsha", script1sha2, "1", "{test}key1", "argv1")); err != nil {
		t.Fatal(err)

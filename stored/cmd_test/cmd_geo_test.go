...
func TestGeoCmds(t *testing.T) {
	...
	for i := 0; i < readNum; i++ {
		...
		if act, err := c.Do("georadius", "Sicil", "15", "37", "200", "km", "WITHDIST", "WITHCOORD", "ASC"); err != nil {
			...
			exp := []interface{}{}
			if !reflect.DeepEqual(exp, act) {
				t.Fatal("georadius fail", act)
			}
		}
	}

	// Test GEORADIUSBYMEMBER operations
	c.Do("del", "Sicily")
	c.Do("del", "StoreKey")
	c.Do("geoadd", "Sicily", "13.361389", "38.115556", "Palermo", "15.087269", "37.502669", "Catania")

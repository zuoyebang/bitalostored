...
const (
	...
	FieldStoreKeyOffset     = 258
	FieldKeyUniqIdOffset    = 260
	FieldFlushIndexOffset   = 268
	FieldMigrateV6Offset    = 276
	FieldSlotInitOffset     = 278

	RemovedSlotGroupOffset = 1024
)
...
func (m *Meta) GetSlotInit() uint16 {
	return m.file.ReadUInt16At(FieldSlotInitOffset)
}

func (m *Meta) SetSlotInit(n uint16) {
	m.file.WriteUInt16At(n, FieldSlotInitOffset)
}

func (m *Meta) GetSnapshotOrder() uint64 {
	return m.file.ReadUInt64At(FieldSnapshotOrderOffset)
}
...
func (m *Meta) GetSnapshotIndex() uint64 {
	...
	order := m.file.ReadUInt64At(FieldSnapshotOrderOffset)
...
func (m *Meta) SetSnapshotIndex(u uint64) uint64 {
	...
	order := m.file.ReadUInt64At(FieldSnapshotOrderOffset) + 1
	offset := FieldSnapshotOffset + FieldSnapshotLength*(order%FieldSnapshotCount)
	now := time.Now().Unix()
	last := m.file.ReadUInt64At(int(offset))
	log.Infof("SetSnapshotIndex index:%d order:%d offset:%d nowTime:%d last:%d", u, order, offset, now, last)

	m.file.WriteUInt64At(order, FieldSnapshotOrderOffset)
...
func (m *Meta) IsMigrateV6Completed() bool {
	return m.file.ReadUInt16At(FieldMigrateV6Offset) == 1
}

func (m *Meta) SetMigrateV6Completed() {
	m.file.WriteUInt16At(1, FieldMigrateV6Offset)
}

func (m *Meta) ClearSnapshot() {
	for i := FieldSnapshotOrderOffset; i < FieldMigrateOffset; i += FieldUIntLenth {
...
func (m *Meta) GetBitalosdbStoreKeyCfg() (isSet bool, t bool) {
	u := m.file.ReadUInt16At(FieldStoreKeyOffset)
	if u == 0 {
		return false, false
	} else if u == 1 {
		return true, true
...
func (m *Meta) SetBitalosdbStoreKeyCfg(t uint16) {
	m.file.WriteUInt16At(t, FieldStoreKeyOffset)
}
...
func (m *Meta) GetMarkedSlots() []int {
	m.mu.RLock()
	defer m.mu.RUnlock()

	var offset int
	marked := make([]int, 0, 100)
	for id := 0; id < int(btools.TotalSlot); id++ {
		offset = RemovedSlotGroupOffset + id
		if m.file.ReadUInt8At(offset) == btools.SlotRemoveStart {
...
func (m *Meta) MarkSlotRemoved(sid, eid int) {
	m.mu.Lock()
	defer m.mu.Unlock()
	for id := sid; id <= eid; id++ {
		m.SetRemoveSlot(id, btools.SlotRemoveStart)
	}
}

func (m *Meta) SetRemoveSlot(slotId int, f uint8) {
	offset := RemovedSlotGroupOffset + slotId
	m.file.WriteUInt8At(f, offset)
}

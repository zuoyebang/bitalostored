...
func TestDB_ScanExpireData(t *testing.T) {
	...
	keyVerMap := make(map[string]uint64, 0)
	...
	skeyStaleVersion := base.EncodeKeyVersion(bdb.SetObj.GetCurrentKeyId(), keyKind)
	...
	zskeyStaleVersion := base.EncodeKeyVersion(bdb.ZsetObj.GetCurrentKeyId(), keyKind)
	...
	require.Equal(t, uint64(80), bdb.baseDb.DelMetaDbNum.Load())
	require.Equal(t, uint64(80), bdb.baseDb.DelExpireDbNum.Load())
	require.Equal(t, uint64(8000), bdb.baseDb.DelDataDbNum.Load())
	require.Equal(t, uint64(4000), bdb.baseDb.DelIndexDbNum.Load())
	...
	var vaildExpire bool
	it := bdb.SetObj.DataDb.NewIterator(nil)
	defer it.Close()
	for it.First(); it.Valid(); it.Next() {
		ver, fp := base.DecodeSetDataKey(keyKind, it.RawKey(), it.RawValue())
		if ver == skeyStaleVersion {
			vaildExpire = true
		} else {
			setMember := fp.Merge()
			keyi, err := strconv.Atoi(strings.Split(string(setMember), "_")[2])
			if err != nil {
				t.Fatal(err)
			}
			if keyi < 40 {
				t.Fatalf("set delkey member exist, f: %s", string(setMember))
			}
		}
	}
	if !vaildExpire {
		t.Fatalf("set stale key not exist")
	}

	vaildExpire = false
	zsIndexIt := bdb.ZsetObj.DataDb.NewIteratorIndex(nil)
	defer zsIndexIt.Close()
	for zsIndexIt.First(); zsIndexIt.Valid(); zsIndexIt.Next() {
		ver, _, fp := base.DecodeZsetIndexKey(keyKind, zsIndexIt.RawKey(), zsIndexIt.RawValue())
		if ver == zskeyStaleVersion {
			vaildExpire = true
		} else {
			member := fp.Merge()
			keyi, err := strconv.Atoi(strings.Split(string(member), "_")[2])
			if err != nil {
				t.Fatal(err)
			}
			if keyi < 40 {
				t.Fatalf("zset delkey field exist, f: %s", string(member))
			}
		}
	}
	if !vaildExpire {
		t.Fatalf("zset stale key not exist")
	}

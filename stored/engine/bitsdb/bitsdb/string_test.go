...
func testNewBitsDB() *BitsDB {
	...
	return testOpenBitsDb(true, cfg)
}

func testNewBitsDBNoDel() *BitsDB {
	cfg := dbconfig.NewConfigDefault()
	return testOpenBitsDb(false, cfg)
}

func testOpenBitsDb(del bool, cfg *dbconfig.Config) *BitsDB {
	...
	meta, err := dbmeta.OpenMeta(testDBPath)
	if err != nil {
		panic(err)
	}

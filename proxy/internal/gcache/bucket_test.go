...
func TestIncrementOverflowInt(t *testing.T) {
	...
	v := x.(int8)
	if v != -128 {
		t.Error("int8 did not overflow as expected; value:", v)
	}
...
func TestIncrementOverflowUint(t *testing.T) {
	...
	v := x.(uint8)
	if v != 0 {
		t.Error("uint8 did not overflow as expected; value:", v)
	}
}
...
func TestDecrementUnderflowUint(t *testing.T) {
	...
	v := x.(uint8)
	if v != 255 {
		t.Error("uint8 did not underflow as expected; value:", v)
	}
}
...
func benchmarkCacheSet(b *testing.B, exp time.Duration) {
	b.StopTimer()
	tc := NewBucketCache(exp, 10*time.Minute, 10)
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		tc.Set("foo", "bar", DefaultExpiration)
	}
